...
def find_path(start: Coord, destination: Coord, heightmatrix) -> int:
    tentative = {start: []}
    candidates = [(0, start)]
    certain = set()
    ...
    if destination in tentative:
        return len(tentative[destination])
    else:
        return None
...
def main():
    ...
    with open(filename, 'r', encoding='utf-8') as f:
        ...
        while line:
            ...
            for cindex, char in enumerate(line):
                if char == 'S' or char == 'a':
                    inputline.append(1)
                    starts.append(Coord(r=rindex, c=cindex))
                elif char == 'E':
    ...
    results = []
    for start in starts:
        newpath = find_path(start, destination, heightmatrix)
        if newpath:
